...
#   default  = stream-json piped through stream-formatter.py
```
...
Find the line describing `files/stream-formatter.py` (currently: `files/stream-formatter.py — Reads Claude Code stream-json from stdin, formats into readable progress lines`) and either:

- Leave as-is, OR
- Adjust to mention the three output modes offered via `YOLO_OUTPUT`, e.g.:
...
grep -c 'stream-formatter.py' files/entrypoint.sh   # returns 1 (only the default branch pipes through it)
```
</verification>
